...
**Важно** Так как ключом правила являеться код ошибки из АПК сопоставление работает только если при выгрузке ошибок
...
Cвойство `sonar.bsl.acc.createExternalIssues`. Определяет как поступить с ошибками, которые были в файле, нодля них
небыло найдено активного правила. Для фильтрации ошибок поступающих от АПК на строне sonarqube потребуеться отключить
свойство.
